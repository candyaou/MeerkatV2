...
router.route('/groups/:group_id/members/:member_id') 
    ...
    .get(function(req, res) {
        ...
        // var query = model.find({
        //     gropuId: req.params.gropu_id, 
        //     'members.memberId': req.params.member_id
        // }).select('members')
        ...
        query.exec(function (err, member) {
            if (err){
                ...
                var return_member = [];
                for(var i in member.members){
                    if(member.members[i].memberId == "TtttAaaa"){
                        return_member.push(member.members[i]);
                    }
                }
                // console.log(return_member)
                // res.json(member)
                res.json(return_member)
            }
        });
    })
...
router.route('/groups/:group_id/members/:member_id/personImages') //POST
...
router.route('/analyze/:group_id') //POST
